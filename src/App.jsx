...
// 관망(미진입): 매수가·매도가가 모두 비어있는 기록 = 실제 진입 없이 셋업/정답매매만 남긴 복기용 기록
const isObserveTrade = (t) => !String(t?.buyPrice ?? "").trim() && !String(t?.sellPrice ?? "").trim();
...
const extractKakaoText = async (file) => {
  ...
  const raw = await claude(
    ...
    3000, undefined, "claude-opus-4-8"
  );
  return (raw || "").trim();
};
  ...
  const analyzeDetailTrade = async (tradeArg) => {
    ...
    try {
      ...
      userContent.push({ type: "text", text:
        ...
        `아래 항목을 분석:\n` +
        (observe
          ? `1. 기법 매칭 & 관망 판단: 이 종목·상황이 [적용 기법 강의록]의 진입조건/주의사항에 부합했는지, 그래서 관망이 옳았는지 아니면 진입 기회였는지 강의록 근거로 판단\n`
        ...
        (observe
          ? `4. 관망 판단 복기: 놓친 기회였는지 / 잘 피한 자리였는지 결론과 근거. 다음에 같은 상황에서 진입/관망을 어떻게 결정할지\n`
    ...
    try {
      ...
      const chartNote = imgs.length
        ? `[첨부 차트 이미지 ${imgs.length}장] 이 실전매매(강사 교본)의 차트다. 아래 규칙으로 B/S를 분석할 것:\n` +
          `- 파란/빨간 오각형(B/S 글자)은 강사가 실제 체결한 매수(B)/매도(S) 지점이며, 이 강사의 B/S가 곧 정답매매(모범 답안)다. 차트는 좌→우 시간순(왼쪽=과거).\n` +
          `- 실전매매 차트엔 네모/동그라미(정답매매 표식)가 없다. 정답 타점을 따로 찾지 말고 강사의 B/S를 정답으로 간주할 것.\n` +
  ...
  const generate = async () => {
    ...
    try {
      ...
      const prompt =
        ...
        `## 2. 반복되는 미흡·실수 패턴 (그룹을 가로질러 이 달에 되풀이된 공통 실수 — 예: 손절 지연, 조급한 진입, 특정 기법 편중, 진입 자리를 반복해서 관망 등. 반드시 개별 매매를 2건 이상 인용해 '왜 반복되는지'까지)\n` +
