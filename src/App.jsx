...
function JournalTab({ techniques }) {
  ...
  const analyzeDetailTrade = async () => {
    ...
    try {
      ...
      const isJongbae = selected.technique?.includes("종배");
      const chartMarkupNote = "\n- 차트에 표시된 도형 해석: 파란/빨간 오각형(B/S 글자)은 사용자가 실제로 체결한 매수(B)/매도(S) 지점이다. '세력의 매수/매도 물량 출현' 같은 추측성 해석을 붙이지 말 것. 사각형(네모) 표시는 강의록 기법 기준 정답매매의 이상적 매수(진입) 타점, 동그라미(원/타원) 표시는 정답매매의 이상적 매도(청산) 타점이다. 이 도형들이 보이면 실제 매매(B/S) 위치와 정답매매(네모/동그라미) 위치를 비교해 진입/청산 타이밍 차이를 분석에 활용하고, 보이지 않으면 언급하지 말 것." +
        ...
        (isJongbae ? `\n- [중요: 이 기법은 '종배' 전략이다] 차트에는 전일과 당일 두 개의 거래일이 함께 표시된다. 가로축에서 날짜 라벨(MM/DD 형식)이 두 개 이상 보이면 왼쪽 날짜가 전일(매수 진입일), 오른쪽 날짜가 당일(매도 청산일)이다. 매수(B)는 전일 장 마감 전후에, 매도(S)는 당일 시초 혹은 장중에 이루어진다. 분석 시 전일 차트 흐름(종가 부근 진입 맥락)과 당일 차트 흐름(시초 갭 방향, 청산 타이밍)을 분리하여 각각 언급할 것.` : "");
