  ...
  const analyzeDetailTrade = async () => {
    ...
    try {
      ...
      const chartMarkupNote = "\n- 차트는 반드시 좌→우 시간순으로 읽을 것. 왼쪽 캔들이 과거, 오른쪽 캔들이 현재/미래다. 차트 왼쪽에 있는 과거 고점·저점·급등락은 현재 매매의 직접 원인이 아닌 '사전 맥락'으로만 참고할 것." +
        "\n- 차트에 표시된 도형 해석: 파란/빨간 오각형(B/S 글자)은 사용자가 실제로 체결한 매수(B)/매도(S) 지점이다. B 마커가 차트 오른쪽에 있다면 왼쪽의 모든 캔들은 '매수 이전 역사'이며 그 구간의 가격 움직임을 현재 매매의 근거로 직접 연결하지 말 것. 분석의 기준점은 B 마커 위치이고, B 이전 차트는 진입 맥락 파악용으로만 활용할 것. '세력의 매수/매도 물량 출현' 같은 추측성 해석을 붙이지 말 것. 사각형(네모) 표시는 강의록 기법 기준 정답매매의 이상적 매수(진입) 타점, 동그라미(원/타원) 표시는 정답매매의 이상적 매도(청산) 타점이다. 이 도형들이 보이면 실제 매매(B/S) 위치와 정답매매(네모/동그라미) 위치를 비교해 진입/청산 타이밍 차이를 분석에 활용하고, 보이지 않으면 언급하지 말 것." +
        "\n- 차트에 표시된 % 수치(예: -52.87%, +49.40% 등)는 키움 HTS가 자동으로 표시하는 '해당 시점의 등락률 주석'이다. 이 수치가 B/S 마커 근처에 없고 차트 왼쪽(과거 구간)에 있다면, 그것은 현재 매매의 수익률이 아닌 과거 특정 시점의 가격 변동폭이다. 절대 현재 매매 손익으로 혼동하지 말 것." +
        ...
        (selected.technique === "상따" ? "\n- [상따 기법 규칙] 날짜 라벨이 두 개인 경우, 전일 차트는 상한가로 마감한 것이다. 당일은 상한가 다음날로서 갭상승 또는 보합 출발하는 흐름을 중점 분석할 것. 당일 갭하락 출발인 경우는 '상따'가 아닌 '상한가하락시작' 기법에 해당하므로 혼동하지 말 것." : "") +
        (selected.technique?.startsWith("상한가하락시작") ? "\n- [상한가하락시작 기법 규칙] 날짜 라벨이 두 개인 경우, 전일 차트는 상한가로 마감한 것이다. 당일은 상한가 다음날이나 갭하락 또는 약세 출발하는 흐름이다. 전일 상한가 마감 후 당일 하락 출발 시점의 매매 맥락을 중점 분석할 것." : "");
