...
function JournalTab({ techniques }) {
  ...
  const analyzeDetailTrade = async () => {
    ...
    try {
      ...
      const isEumbongJongbae = selected.technique === "음봉매매-종배";
      const chartMarkupNote = "\n- 차트에 표시된 도형 해석: 파란/빨간 오각형(B/S 글자)은 사용자가 실제로 체결한 매수(B)/매도(S) 지점이다. '세력의 매수/매도 물량 출현' 같은 추측성 해석을 붙이지 말 것. 사각형(네모) 표시는 강의록 기법 기준 정답매매의 이상적 매수(진입) 타점, 동그라미(원/타원) 표시는 정답매매의 이상적 매도(청산) 타점이다. 이 도형들이 보이면 실제 매매(B/S) 위치와 정답매매(네모/동그라미) 위치를 비교해 진입/청산 타이밍 차이를 분석에 활용하고, 보이지 않으면 언급하지 말 것." +
        ...
        (isEumbongJongbae ? "\n- [중요: 이 기법은 '음봉매매-종배' 전략이다] 가로축에서 MM/DD 형식의 날짜 라벨이 두 개 보이는 경우에만 아래 해석을 적용할 것. 날짜 라벨이 하나뿐이면 단일 거래일 차트이므로 이 규칙을 무시할 것. 날짜 라벨이 두 개인 경우: 왼쪽 날짜가 전일, 오른쪽 날짜가 당일이다. 매수(B)는 전일 어느 시점에서든 발생할 수 있다. 매도(S)는 통상 당일에 이루어지나 전일에 청산하는 경우도 있다. 분석 시 차트에서 실제로 확인되는 B/S 위치를 기준으로 전일 흐름과 당일 흐름을 구분하여 각각 언급할 것." : "");
